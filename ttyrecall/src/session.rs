...
mod test {
    ...
    use crate::{
        ...
        indexer::indexed_chunks_for_path,
    ...
    #[test]
    fn zstd_session_is_indexable_after_finalization() {
        ...
        assert!(final_path.exists());
        assert!(!unfinished_path_for(&final_path).exists());
        let chunks = indexed_chunks_for_path(&root, &final_path).unwrap();
        assert_eq!(chunks.len(), 1);
        assert_eq!(chunks[0].text, "hello from unfinished recording");
